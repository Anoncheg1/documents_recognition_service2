...
def h_conv(X, W, strides=(1,1,1,1), padding='VALID', max_order=1, name='h_conv'):
    ...
    name: (default h_conv)
    """
    with tf.compat.v1.name_scope('hconv'+str(name)) as scope:
        ...
        # Convolve
        Y = tf.nn.conv2d(input=X_, filters=W_, strides=strides, padding=padding, name=name)
...
def h_batch_norm(X, fnc, train_phase, decay=0.99, eps=1e-12, name='hbn'):
    ...
    with tf.compat.v1.name_scope(name) as scope:
        ...
        c = tf.math.divide(fnc(Rb), magnitude)
        return c*X
...
def bn(X, train_phase, decay=0.99, name='batchNorm'):
    ...
    with tf.compat.v1.name_scope(name) as scope:
        ...
        pop_var = tf.compat.v1.get_variable(name+'_pop_var', dtype=tf.float32,
                                          shape=n_out, trainable=False)
        batch_mean, batch_var = tf.nn.moments(x=X, axes=list(range(len(Xsh)-3)), name=name+'moments')
        ema = tf.train.ExponentialMovingAverage(decay=decay)


    def mean_var_with_update():
        ema_apply_op = ema.apply([batch_mean, batch_var]) # Error 'Tensor' object has no attribute '_shared_name'
    ...
    # print(train_phase.shape)
    # print(mean_var_with_update.shape)
    mean, var = pop_mean, pop_var
    mean, var = tf.cond(pred=train_phase, true_fn=mean_var_with_update,
                false_fn=lambda: (pop_mean, pop_var))
...
def mean_pooling(x, ksize=(1,1,1,1), strides=(1,1,1,1)):
    ...
    X_ = tf.reshape(x, tf.concat(axis=0,values=[Xsh[:3],[-1]]))
    Y = tf.nn.avg_pool2d(input=X_, ksize=ksize, strides=strides, padding='VALID',
                       name='mean_pooling')
    Ysh = Y.get_shape()
...
def stack_magnitudes(X, eps=1e-12, keep_dims=True):
    ...
    """
    R = tf.compat.v1.reduce_sum(tf.square(X), axis=[4], keepdims=keep_dims)
...
#     return tf.cast(Q, dtype=tf.float32)
...
def get_weights(filter_shape, W_init=None, std_mult=0.4, name='W'):
    ...
    if W_init == None:
        ...
        W_init = tf.compat.v1.random_normal_initializer(stddev=stddev)
...
def get_weights_dict(shape, max_order, std_mult=0.4, n_rings=None, name='W'):
    ...
    if isinstance(max_order, int):
        orders = range(-max_order, max_order+1) #-1 +2   -3 +4
    else:
    ...
    for i in orders:
        if n_rings is None:
            n_rings = np.maximum(shape[0]/2, 2)  # nrings = h/2
        sh = [n_rings,] + shape[2:]  # [4,i,o]
        nm = name + '_' + str(i)  # W_-1 W_0 W_1
