...
from predict_utils.hnet_model_tf1.harmonic_network_ops import *
...
def sum_magnitudes(x, eps=1e-12, keep_dims=True):
    ...
    """
    R = tf.reduce_sum(input_tensor=tf.square(x), axis=[4], keepdims=keep_dims)
    return tf.reduce_sum(input_tensor=tf.sqrt(tf.maximum(R,eps)), axis=[3], keepdims=keep_dims)
